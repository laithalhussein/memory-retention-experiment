...
num_norm_EC_trials = 3; %how many error clamp trials to normalize by
num_norm_EC_trials_AS = 1;
...
pAS_norm_val = nanmean(pAS_avg(end-30+1-num_norm_EC_trials_AS+1:end-30+1));
...
vAS_norm_val = nanmean(vAS_avg(end-30+1-num_norm_EC_trials_AS+1:end-30+1));
...
pGL_ret_std = nanstd(pGL(:,end-29:end),0,1);

pAL_ret_norm = pAL(:,end-29:end)/pAL_norm_val;
pAL_ret_norm_avg = nanmean(pAL_ret_norm,1);
pAL_ret_norm_std = nanstd(pAL_ret_norm,0,1);
pAL_ret_std = nanstd(pAL(:,end-29:end),0,1);

pAS_ret_norm = pAS(:,end-29:end)/pAS_norm_val;
pAS_ret_norm_avg = nanmean(pAS_ret_norm,1);
pAS_ret_norm_std = nanstd(pAS_ret_norm,0,1);
pAS_ret_std = nanstd(pAS(:,end-29:end),0,1);

vGL_ret_norm = vGL(:,end-29:end)/vGL_norm_val;
vGL_ret_norm_avg = nanmean(vGL_ret_norm,1);
vGL_ret_norm_std = nanstd(vGL_ret_norm,0,1);
vGL_ret_std = nanstd(vGL(:,end-29:end),0,1);

vAL_ret_norm = vAL(:,end-29:end)/vAL_norm_val;
vAL_ret_norm_avg = nanmean(vAL_ret_norm,1);
vAL_ret_norm_std = nanstd(vAL_ret_norm,0,1);
vAL_ret_std = nanstd(vAL(:,end-29:end),0,1);

vAS_ret_norm = vAS(:,end-29:end)/vAS_norm_val;
vAS_ret_norm_avg = nanmean(vAS_ret_norm,1);
vAS_ret_norm_std = nanstd(vAS_ret_norm,0,1);
vAS_ret_std = nanstd(vAS(:,end-29:end),0,1);

%% plot raw decay (pFF)

figure; 
plot(trials,nanmean(pGL(:,end-29:end),1)*100,'g', 'linewidth', 2); hold on;
plot(trials,nanmean(pAL(:,end-29:end),1)*100,'r', 'linewidth', 2);
plot(trials,nanmean(pAS(:,end-29:end),1)*100,'b', 'linewidth', 2);

standard_error_shading_11_07_14(nanmean(pGL(:,end-29:end),1)*100,pGL_ret_std*100,trials,13,'g');
standard_error_shading_11_07_14(nanmean(pAL(:,end-29:end),1)*100,pAL_ret_std*100,trials,14,'r');
standard_error_shading_11_07_14(nanmean(pAS(:,end-29:end),1)*100,pAS_ret_std*100,trials,14,'b');
xlabel('Trials');
...
figure; 
plot(trials,nanmean(vGL(:,end-29:end),1)*100,'g', 'linewidth', 2); hold on;
plot(trials,nanmean(vAL(:,end-29:end),1)*100,'r', 'linewidth', 2);
plot(trials,nanmean(vAS(:,end-29:end),1)*100,'b', 'linewidth', 2);

standard_error_shading_11_07_14(nanmean(vGL(:,end-29:end),1)*100,vGL_ret_std*100,trials,8,'g');
standard_error_shading_11_07_14(nanmean(vAL(:,end-29:end),1)*100,vAL_ret_std*100,trials,8,'r');
standard_error_shading_11_07_14(nanmean(vAS(:,end-29:end),1)*100,vAS_ret_std*100,trials,8,'b');
xlabel('Trials');
ylabel('Adaptation Coefficient');
...
%% position normalized decay

% figure; 
% plot(trials,pGL_ret_norm_avg*100,'g', 'linewidth', 2); hold on;
% plot(trials,pAL_ret_norm_avg*100,'r', 'linewidth', 2);
% plot(trials,pAS_ret_norm_avg*100,'b', 'linewidth', 2);
% 
% standard_error_shading_11_07_14(pGL_ret_norm_avg*100,pGL_ret_norm_std*100,trials,13,'g');
% standard_error_shading_11_07_14(pAL_ret_norm_avg*100,pAL_ret_norm_std*100,trials,14,'r');
% standard_error_shading_11_07_14(pAS_ret_norm_avg*100,pAS_ret_norm_std*100,trials,14,'b');
% xlabel('Trials');
% ylabel('% Retention');
% 
% ylim([-20,100]);
...
p_early_avg = [mean(pAL_ret_norm_avg(early)), mean(pGL_ret_norm_avg(early)), mean(pAS_ret_norm_avg(early))]*100;
p_early_std = [std(pAL_ret_norm_avg(early)), std(pGL_ret_norm_avg(early)), std(pAS_ret_norm_avg(early))]*100;

p_late_avg = [mean(pAL_ret_norm_avg(late)), mean(pGL_ret_norm_avg(late)), mean(pAS_ret_norm_avg(late))]*100;
p_late_std = [std(pAL_ret_norm_avg(late)), std(pGL_ret_norm_avg(late)), std(pAS_ret_norm_avg(late))]*100;
...
p_late_se=[[nan,nan,nan]',p_late_std'];
...
%% velocity normalized decay

% figure; 
% plot(trials,vGL_ret_norm_avg*100,'g', 'linewidth', 2); hold on;
% plot(trials,vAL_ret_norm_avg*100,'r', 'linewidth', 2);
% plot(trials,vAS_ret_norm_avg*100,'b', 'linewidth', 2);
% 
% standard_error_shading_11_07_14(vGL_ret_norm_avg*100,vGL_ret_norm_std*100,trials,8,'g');
% standard_error_shading_11_07_14(vAL_ret_norm_avg*100,vAL_ret_norm_std*100,trials,8,'r');
% standard_error_shading_11_07_14(vAS_ret_norm_avg*100,vAS_ret_norm_std*100,trials,8,'b');
% xlabel('Trials');
% ylabel('% Retention');
% 
% title('Velocity data');
...
v_early_avg = [mean(vAL_ret_norm_avg(early)), mean(vGL_ret_norm_avg(early)), mean(vAS_ret_norm_avg(early))]*100;
v_early_std = [std(vAL_ret_norm_avg(early)), std(vGL_ret_norm_avg(early)), std(vAS_ret_norm_avg(early))]*100;

v_late_avg = [mean(vAL_ret_norm_avg(late)), mean(vGL_ret_norm_avg(late)), mean(vAS_ret_norm_avg(late))]*100;
v_late_std = [std(vAL_ret_norm_avg(late)), std(vGL_ret_norm_avg(late)), std(vAS_ret_norm_avg(late))]*100;
...
v_early_se=[[nan,nan,nan]',v_early_std'*8.^-.5];

v_late_avg=[[nan,nan,nan]',v_late_avg'];
% v_late_se=[[nan,nan,nan]',v_late_std'*8.^-.5];
v_late_se=[[nan,nan,nan]',v_late_std'];
...
p_vel_early_ret = anova1(vel_early_ret_mat);
p_vel_late_ret = anova1(vel_late_ret_mat);

%velocity, normalized
p_vel_early_ret_norm = anova1(vel_early_ret_norm_mat);
p_vel_late_ret_norm = anova1(vel_late_ret_norm_mat);


%% ANOVAS for time constants

%load the time constans
load('sub_time_constants_01_02_2019.mat')

%put them in matrices for comparison
ptau_mat = [glp_tau; alp_tau; asp_tau]';
vtau_mat = [glv_tau; alv_tau; asv_tau]';

%do the comparison
p_ptau = anova1(ptau_mat);
p_vtau = anova1(vtau_mat);
