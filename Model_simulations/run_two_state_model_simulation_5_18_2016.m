...
ast1=as_total(1:30); ass1=as_slow(1:30); asf1=(as_fast(1:16 

plot([1-15:length(mean(Abrupt_short_output))-15],mean(Abrupt_short_output)','color',abrupt_short_adapt_color,'Displayname','Net Motor Output','Linewidth',curve_width);
plot([1-15:length(mean(Abrupt_short_slow))-15],mean(Abrupt_short_slow)','color','k','Displayname','Slow State','Linewidth',1.5);
plot([1-15:length(mean(Abrupt_short_fast))-15],mean(Abrupt_short_fast)','color',[0.5,0.5,0.5],'Displayname','Fast State','Linewidth',1.5);
legend('Show');
title('Abrupt Short','Fontsize',12);
o1=plot([-15,length(mean(Abrupt_short_output))-15],[0,0],'k--','lineWidth',.5);
hg=get(o1,'Annotation');hLegendEntry = get(hg,'LegendInformation');set(hLegendEntry,'IconDisplayStyle','off');
%
%
h4=area([65,75],[1.01,1.01],'linestyle','none','facecolor',late_wash_shade,'basevalue',-.2);
hg=get(h4,'Annotation');hLegendEntry = get(hg,'LegendInformation');set(hLegendEntry,'IconDisplayStyle','off');uistack(h4,'bottom');
%
set(gca,'FontSize',12,'xtick',[0:15:75],'ytick',y,'ylim',[-0.2,1.01],'xlim',[-15,76],'layer','top');
...
o1=plot([0,length(mean(Abrupt_long_output))-15],[0,0],'k--','lineWidth',.5);
hg=get(o1,'Annotation');hLegendEntry = get(hg,'LegendInformation');set(hLegendEntry,'IconDisplayStyle','off');
%
%
h4=area([225-15,235-15],[1.2,1.2],'linestyle','none','facecolor',late_wash_shade,'basevalue',-.2);
hg=get(h4,'Annotation');hLegendEntry = get(hg,'LegendInformation');set(hLegendEntry,'IconDisplayStyle','off');uistack(h4,'bottom');
%
set(gca,'FontSize',12,'xtick',[0:50:200],'ytick',y,'ylim',[-0.2,1.01],'xlim',[-15,221],'layer','top');
xlabel('Trial Number','FontSize',12,'FontWeight','normal','Color','k');
ylabel('Adaptation Coefficient','FontSize',12,'FontWeight','normal','Color','k');
% 
