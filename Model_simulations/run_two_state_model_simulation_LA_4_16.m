...
%wash_period_abrupt_long=[61,120];
...
%F_gradual=[zeros(9,1);linspace(0,1,46)';ones(5,1)];
wash_period_gradual=[176,235];
%wash_period_gradual=[61,120];
...
for nn=1:iteration;
    As=.97;
    Bf=.2;
    pick=randi(length(R_ratio));
     Af=As*R_ratio(pick);
    Bs=Bf*R_ratio(randi(length(R_ratio)));
    %Bs=Bf*R_ratio(length(R_ratio)-pick+1);
    % Af=As*R_ratio(nn);
    % Bs=Bf*R_ratio(iteration-nn+1); 
    A=[As 0;0, Af];
    B=[Bs;Bf];
    R=1e-2;
    Q=sqrt(3)*10^-2;
...
Abrupt_short_early_wash=Abrupt_short_output(:,end-49:end-40);
Abrupt_long_early_wash=Abrupt_long_output(:,end-49:end-40);
Gradual_early_wash=Gradual_output(:,end-49:end-40);
%
Abrupt_short_mid_wash=Abrupt_short_output(:,end-29:end-20);
Abrupt_long_mid_wash=Abrupt_long_output(:,end-29:end-20);
Gradual_mid_wash=Gradual_output(:,end-29:end-20);
%
...
%
data_early_wash_all={Abrupt_short_early_wash(:),Abrupt_long_early_wash(:),Gradual_early_wash(:)};
data_mid_wash_all={Abrupt_short_mid_wash(:),Abrupt_long_mid_wash(:),Gradual_mid_wash(:)};
data_late_wash_all={Abrupt_short_late_wash(:),Abrupt_long_late_wash(:),Gradual_late_wash(:)};
%
fit_data_early_wash_ave_all=cellfun(@nanmean,data_early_wash_all);
fit_data_mid_wash_ave_all=cellfun(@nanmean,data_mid_wash_all);
fit_data_late_wash_ave_all=cellfun(@nanmean,data_late_wash_all);
%
fit_data_early_wash_sd_all=cellfun(@nanstd,data_early_wash_all);
fit_data_mid_wash_sd_all=cellfun(@nanstd,data_mid_wash_all);
fit_data_late_wash_sd_all=cellfun(@nanstd,data_late_wash_all);
%
[h_matrix_fit_early_wash_all,p_matrix_fit_early_wash_all]=calculate_t_test_for_subject_data_13_07_14(data_early_wash_all);
[h_matrix_fit_mid_wash_all,p_matrix_fit_mid_wash_all]=calculate_t_test_for_subject_data_13_07_14(data_mid_wash_all);
[h_matrix_fit_late_wash_all,p_matrix_fit_late_wash_all]=calculate_t_test_for_subject_data_13_07_14(data_late_wash_all);
%% Normalized%% 
Abrupt_short_early_wash_norm=Abrupt_short_output(:,end-49:end-40)'/diag(Abrupt_short_output(:,end-59));
Abrupt_long_early_wash_norm=Abrupt_long_output(:,end-49:end-40)'/diag(Abrupt_long_output(:,end-59));
Gradual_early_wash_norm=Gradual_output(:,end-49:end-40)'/diag(Gradual_output(:,end-59));
%
Abrupt_short_mid_wash_norm=Abrupt_short_output(:,end-29:end-20)'/diag(Abrupt_short_output(:,end-59));
Abrupt_long_mid_wash_norm=Abrupt_long_output(:,end-29:end-20)'/diag(Abrupt_long_output(:,end-59));
Gradual_mid_wash_norm=Gradual_output(:,end-29:end-20)'/diag(Gradual_output(:,end-59));
%
...
%
data_early_wash_all_norm={Abrupt_short_early_wash_norm(:),Abrupt_long_early_wash_norm(:),Gradual_early_wash_norm(:)};
data_mid_wash_all_norm={Abrupt_short_mid_wash_norm(:),Abrupt_long_mid_wash_norm(:),Gradual_mid_wash_norm(:)};
data_late_wash_all_norm={Abrupt_short_late_wash_norm(:),Abrupt_long_late_wash_norm(:),Gradual_late_wash_norm(:)};
%
fit_data_early_wash_ave_all_norm=cellfun(@nanmean,data_early_wash_all_norm);
fit_data_mid_wash_ave_all_norm=cellfun(@nanmean,data_mid_wash_all_norm);
fit_data_late_wash_ave_all_norm=cellfun(@nanmean,data_late_wash_all_norm);
%
fit_data_early_wash_sd_all_norm=cellfun(@nanstd,data_early_wash_all_norm);
fit_data_mid_wash_sd_all_norm=cellfun(@nanstd,data_mid_wash_all_norm);
fit_data_late_wash_sd_all_norm=cellfun(@nanstd,data_late_wash_all_norm);
%
[h_matrix_fit_early_wash_all_norm,p_matrix_fit_early_wash_all_norm]=calculate_t_test_for_subject_data_13_07_14(data_early_wash_all_norm);
[h_matrix_fit_mid_wash_all_norm,p_matrix_fit_mid_wash_all_norm]=calculate_t_test_for_subject_data_13_07_14(data_mid_wash_all_norm);
...
%
h4=area([washout_trial(early_wash_st),washout_trial(early_wash_st+window_length)],[1.2,1.2],'linestyle','none','facecolor',early_wash_shade,'basevalue',-.2);
hg=get(h4,'Annotation');hLegendEntry = get(hg,'LegendInformation');set(hLegendEntry,'IconDisplayStyle','off');uistack(h4,'bottom');
%
h4=area([washout_trial(mid_wash_st),washout_trial(mid_wash_st+window_length)],[1.2,1.2],'linestyle','none','facecolor',mid_wash_shade,'basevalue',-.2);
hg=get(h4,'Annotation');hLegendEntry = get(hg,'LegendInformation');set(hLegendEntry,'IconDisplayStyle','off');uistack(h4,'bottom');
...
hg=get(h4,'Annotation');hLegendEntry = get(hg,'LegendInformation');set(hLegendEntry,'IconDisplayStyle','off');uistack(h4,'bottom');
%
text(washout_trial(early_wash_st+floor(window_length/2)),-0.05,'E','VerticalAlignment','top','HorizontalAlignment','center','FontSize',7);
text(washout_trial(mid_wash_st+floor(window_length/2)),-0.05,'M','VerticalAlignment','top','HorizontalAlignment','center','FontSize',7);
...
plot_bar_with_error_11_07_14(x',fit_early_adapt_mid_wash_norm_ave',fit_early_adapt_mid_wash_norm_se',Training_sequence,Training_color_seq,h_matrix_fit_early_wash_all,h_matrix_fit_mid_wash_all);
%
...
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
subplot('position',[.86,.15,.12,.2])
fit_early_adapt_mid_wash_norm_ave=[fit_data_early_wash_ave_all_norm',fit_data_mid_wash_ave_all_norm'];
fit_early_adapt_mid_wash_norm_se=[fit_data_early_wash_sd_all_norm',fit_data_mid_wash_sd_all_norm'];
x=[.25 .75;.25 .75;.25 .75];
Training_sequence={'abrupt short','abrupt long','gradual'};
Training_color_seq=[abrupt_short_adapt_color;abrupt_long_adapt_color;gradual_adapt_color];
plot_bar_with_error_11_07_14(x',fit_early_adapt_mid_wash_norm_ave',fit_early_adapt_mid_wash_norm_se',Training_sequence,Training_color_seq,h_matrix_fit_early_wash_all_norm,h_matrix_fit_mid_wash_all_norm);
%
...
Training_sequence={'abrupt short','abrupt long','gradual'};
Training_color_seq=[abrupt_short_adapt_color;abrupt_long_adapt_color;gradual_adapt_color];
plot_bar_with_error_11_07_14(x',fit_late_wash_ave',fit_late_wash_se',Training_sequence,Training_color_seq,zeros(3),h_matrix_fit_late_wash_all_norm);
